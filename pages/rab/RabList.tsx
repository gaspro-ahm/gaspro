...
interface ActionMenuProps {
    rab: RabDocument;
    onEdit: (rab: RabDocument) => void;
    onDelete: (id: string) => void;
}

const ActionMenu = ({ rab, onDelete, onEdit }: ActionMenuProps) => {
        ...
        <div className="relative" ref={menuRef}>
            ...
            {isOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-xl z-20 animate-fade-in-up-fast">
                    <Link to={`/rab/detail/${rab.id}`} className="flex items-center w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"><Eye size={16} className="mr-2"/> Lihat Detail</Link>
                    <button onClick={() => { onEdit(rab); setIsOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700"><Pencil size={16} className="mr-2"/> Edit RAB</button>
                    <button onClick={() => { onDelete(rab.id); setIsOpen(false); }} className="w-full text-left flex items-center px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-gray-100 dark:hover:bg-gray-700"><Trash2 size={16} className="mr-2"/> Hapus</button>
                </div>
            )}
        </div>
    )
...
const calculateRowTotalBudget = (doc: RabDocument) => {
    return doc.detailItems.reduce((sum, item) => sum + (item.volume * item.hargaSatuan), 0);
};

const calculatePercentageDifference = (rabValue: number, tenderValue: number | null): { value: string; color: string } => {
    if (rabValue === 0 || tenderValue === null) {
        return { value: '-', color: 'text-gray-800 dark:text-gray-100' };
    }
...
const RabRow = React.memo(({ rab, index, onEdit, onDelete, onMove, totalRows } : { rab: RabDocument, index: number, onEdit: (rab: RabDocument) => void, onDelete: (id: string) => void, onMove: (index: number, direction: 'up' | 'down') => void, totalRows: number }) => {
    ...
    return (
        <tr className={`group bg-white dark:bg-gray-800 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors`}>
            ...
            <th scope="row" className="px-4 py-2 font-medium text-gray-900 dark:text-white whitespace-nowrap text-center text-xs">{rab.eMPR}</th>
            <td className="px-4 py-2 text-xs">
                <Link to={`/rab/detail/${rab.id}`} className="hover:text-honda-red hover:underline transition-colors">
                    {rab.projectName}
                </Link>
            </td>
            ...
            <td className="px-4 py-2 text-xs">{rab.keterangan || '-'}</td>
            <td className="px-4 py-2 text-center">
                <ActionMenu rab={rab} onDelete={onDelete} onEdit={onEdit} />
            </td>
        </tr>
    );
});
...
const RabList = () => {
  ...
  const [localRabData, setLocalRabData] = useState<RabDocument[]>([]);
  const [isOrderChanged, setIsOrderChanged] = useState(false);
  
  useEffect(() => {
    setLocalRabData(rabData);
  ...
  const activeFilterCount = useMemo(() => {
    ...
    return count;
  }, [statusFilters, dateFilters, slaFilter, percentageFilter]);

  const sortedAndFilteredData = useMemo(() => {
    let data = [...localRabData];
    data = data.filter(rab => {
        const matchesSearch = rab.eMPR.toLowerCase().includes(searchTerm.toLowerCase()) || rab.projectName.toLowerCase().includes(searchTerm.toLowerCase());
        const matchesStatus = statusFilters.length === 0 || statusFilters.includes(rab.status);
        ...
        const matchesPercentage = (() => {
            if (percentageFilter.min === '' && percentageFilter.max === '') return true;
            if (totalBudget === 0 || rab.tenderValue === null) return false;
            const percentageValue = ((rab.tenderValue - totalBudget) / totalBudget) * 100;
            const minPct = percentageFilter.min !== '' ? parseFloat(percentageFilter.min) : -Infinity;
            const maxPct = percentageFilter.max !== '' ? parseFloat(percentageFilter.max) : Infinity;
            return percentageValue >= minPct && percentageValue <= maxPct;
        })();
        
        return matchesSearch && matchesStatus && matchesDates && matchesSla && matchesPercentage;
    });
    ...
    if (sortConfig) {
      data.sort((a, b) => {
        ...
        if (sortConfig.key === 'sla') { aValue = calculateSla(a.receivedDate, a.finishDate); bValue = calculateSla(b.receivedDate, b.finishDate); if (aValue === '-') aValue = -1; if (bValue === '-') bValue = -1; } 
            ...
            const bRab = calculateRowTotalBudget(b);
            aValue = (aRab === 0 || a.tenderValue === null) ? -Infinity : ((a.tenderValue - aRab) / aRab);
            bValue = (bRab === 0 || b.tenderValue === null) ? -Infinity : ((b.tenderValue - bRab) / bRab);
        }
    ...
    return data;
  }, [localRabData, searchTerm, statusFilters, dateFilters, slaFilter, percentageFilter, sortConfig]);
  ...
  return (
    <>
      <ConfirmationModal isOpen={isConfirmOpen} onClose={() => setIsConfirmOpen(false)} onConfirm={confirmDelete} title="Hapus RAB" message="Apakah Anda yakin ingin menghapus data RAB ini secara permanen? Tindakan ini tidak dapat dibatalkan." />
      <CreateRabModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingRab(null); }} onSave={handleSaveRab} initialData={editingRab} />
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md dark:border dark:border-gray-700">
        <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
            ...
            <div className="relative" ref={filterRef}>
              ...
              {isFilterOpen && (
                 <div className="absolute left-0 mt-2 w-80 bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-xl z-20 animate-fade-in-up-fast p-4 space-y-4">
                    ...
                    <div className="border-t border-gray-200 dark:border-gray-700 pt-3 space-y-3">
                     ...
                     <div className="border-t border-gray-200 dark:border-gray-700 pt-3">
                        <h4 className="text-sm font-semibold mb-2 text-gray-800 dark:text-gray-100">% Selisih</h4>
                        <div className="flex items-center gap-2">
                             <input type="number" placeholder="Min" value={percentageFilter.min} onChange={(e) => setPercentageFilter(p => ({ ...p, min: e.target.value }))} className={numberInputClasses} />
                            <span className="text-gray-400">-</span>
                            <input type="number" placeholder="Max" value={percentageFilter.max} onChange={(e) => setPercentageFilter(p => ({ ...p, max: e.target.value }))} className={numberInputClasses} />
                        </div>
                    </div>
             ...
             <div className="flex items-center gap-2 border-l border-gray-300 dark:border-gray-600 pl-2 ml-2">
                ...
                <select id="sort-by" value={sortConfig?.key || ''} onChange={(e) => handleSortKeyChange(e.target.value as SortKey)} className="text-sm p-2 border-gray-300 dark:border-gray-600 rounded-lg focus:ring-1 focus:ring-honda-red focus:border-transparent transition bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200">
                    ...
                    <option value="totalBudget">Nilai RAB</option>
                    <option value="tenderValue">Nilai Tender</option>
                    <option value="percentageDifference">% Selisih</option>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleOpenCreateModal} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-white bg-black dark:bg-gray-200 dark:text-black rounded-lg hover:bg-gray-800 dark:hover:bg-white transition shadow"><Plus size={16} /> Buat RAB</button>
          </div>
        </div>
          ...
          <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400 border-collapse">
            <thead className="text-xs text-gray-700 dark:text-gray-300 uppercase bg-gray-50 dark:bg-gray-700/50 sticky top-0 z-10">
              <tr>
                ...
                <th scope="col" className={`${headerButtonClass} min-w-[150px]`} onClick={() => handleHeaderSort('totalBudget')}>Nilai RAB {getSortIndicator('totalBudget')}</th>
                <th scope="col" className={`${headerButtonClass} min-w-[150px]`} onClick={() => handleHeaderSort('tenderValue')}>Nilai Tender {getSortIndicator('tenderValue')}</th>
                <th scope="col" className={`${headerButtonClass} min-w-[120px]`} onClick={() => handleHeaderSort('percentageDifference')}>% Selisih {getSortIndicator('percentageDifference')}</th>
            ...
            <tbody>
              {sortedAndFilteredData.map((rab, index) => (
                <RabRow key={rab.id} rab={rab} index={index} onEdit={handleOpenEditModal} onDelete={handleDeleteRequest} onMove={handleMoveRow} totalRows={sortedAndFilteredData.length} />
              ))}
            </tbody>
          </table>
        </div>
